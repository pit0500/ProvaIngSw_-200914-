	...
	public int stringToIntConverter(String number) throws UnsupportedOperationException {
		if (number.charAt(number.length()-1) == ' ') {
			...
			for (int i = 0; i < number.length()-1; ++i)
				tmparr[i] = number.charAt(i);
			String tmp = new String(tmparr);
			int num = Integer.parseInt(tmp);
			if (num > Short.MAX_VALUE || num < Short.MIN_VALUE)
				throw new IllegalArgumentException("La stringa passata contiene un numero fuori dal range. Passare un numero nell'intervallo [-32768, 32767]");
			return num;
		}
